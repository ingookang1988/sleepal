...
import { applyViewBox, blinkAmount, EYE, EYE_RX, F, getScreenWmm, lidPath, lowLidPath, ORDER,
...
function frame(ts) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    // 라운드 사각 눈 — rx 상한 12(목업), 찡그려 낮아지면 hi/2 가 이겨 알약이 된다
    eye.setAttribute('rx', Math.min(EYE_RX, hi / 2).toFixed(2));
